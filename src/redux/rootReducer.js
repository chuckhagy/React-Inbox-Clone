export default function rootReducer(
  ...
  switch (action.type) {
    ...
    case "MARK_AS_READ":
      ...
      newMessages.find(
        thisMessage => thisMessage.id === action.messageId
      ).read = true;
      return {
        ...currentState,
    ...
    case "REMOVE_LABEL_SELECTED":
      ...
      let theseLabels = newMessagesCutLabels
        .find(message => message.id === action.id)
        .labels.filter(thisLabel => thisLabel !== action.label);
      newMessagesCutLabels.find(
    ...
    case "DELETE":
      let newSelectedDelete = currentState.selected.filter(
        id => id !== action.messageId
      );
      let postDeleteMessages = currentState.messages.filter(
